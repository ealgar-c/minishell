...
/*   Updated: 2023/09/27 13:07:14 by ealgar-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_redir(t_lexer *lexer, t_parser *parser)
{
	...
	token = lexer->token;
	if (token == GREAT)
		parser->redir_out = open(lexer->next->content, O_CREAT, O_WRONLY, O_TRUNC);
	else if (token == LESS)
		parser->redir_in = open(lexer->next->content, O_RDONLY);
	...
	{
		parser->redir_out = open(lexer->next->content, O_WRONLY | O_CREAT | O_APPEND);
